class Book {
    constructor(title, author, pubDate) {
      this.title = title;
      this.author = author;
      this.pubDate = pubDate;
    }
  }

class Library {
    constructor(name) {
      this._name = name;
      this._books = [];
    }
    get books() {
      // Return copy of books
      return JSON.parse(JSON.stringify(this._books));
    }
    get count() {
      return this._books.length;
    }
    addBook(book = {}) {
      const { title = "", author = "", pubDate = "" } = book;
      if (title.length > 0 && author.length > 0) {
        const newBook = { title, author, pubDate };
        this._books.push(newBook);
      }
    }

    deleteBook(isbn) {
        const index = this._books.findIndex((book) => book.isbn === isbn);
        if (index !== -1) {
          this._books.splice(index, 1);
        }
      }

    listBooks() {
      for (const book of this._books) {
        const {title, author, pubDate, isbn} = book;
        console.log(`Title: ${title}, Author: ${author}, PubDate: ${pubDate}`)
      }
    }
  }

  // Creating a new library instance
  const myLibrary = new Library("New York Times Best Seller List");
  
// Create a book
// const atomicHabits = new Book("Atomic Habits", "James Clear", "10/16/2018");
// const onlyTheDead = new Book("Only The Dead", "Jack Carr", "05/15/2023");
// const happyPlaces = new Book("Happy Place", "Emily Henry", "04/25/2023");

// Adding books to the library
  //myLibrary.addBook(atomicHabits);
  //myLibrary.addBook(onlyTheDead);
  //myLibrary.addBook(happyPlaces);

  // Create books
const atomicHabits = new Book("Atomic Habits", "James Clear", "10/16/2018", "0735211299");
const theHillWeClimb = new Book("The Hill We Climb", "David Baldacci", "03/30/2021", "059346527X");
const oceanPrey = new Book("Atomic Habits", "John Sandford", "04/13/2021", "1398505501");

// Add books to library and output library count and books
myLibrary.addBook(atomicHabits);
myLibrary.addBook(theHillWeClimb);
myLibrary.addBook(oceanPrey);
console.log(`Book count: ${myLibrary.count}`);
myLibrary.listBooks();

// Delete a book and output library books
console.log("* Library after delete *");
myLibrary.deleteBook("059346527X");
myLibrary.listBooks();
  
  // Listing books in the library
  myLibrary.listBooks();
  
  // Getting the count of books in the library
  console.log(`"Book count:", ${myLibrary.count}`);
  
